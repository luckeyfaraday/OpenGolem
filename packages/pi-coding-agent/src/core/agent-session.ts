...
export class AgentSession {
	...
	private _isRetryableError(message: AssistantMessage): boolean {
		...
		return classifyRetryableError(message.errorMessage) !== "none";
	}

	private _getRetryLimit(message: AssistantMessage, settings: ReturnType<SettingsManager["getRetrySettings"]>): number {
		return getRetryLimit(classifyRetryableError(message.errorMessage || ""), settings.maxRetries);
	}

	private _getRetryDelayMs(message: AssistantMessage, settings: ReturnType<SettingsManager["getRetrySettings"]>): number {
		return getRetryDelayMs({
			kind: classifyRetryableError(message.errorMessage || ""),
			attempt: this._retryAttempt,
			baseDelayMs: settings.baseDelayMs,
			maxDelayMs: settings.maxDelayMs ?? Number.POSITIVE_INFINITY,
	...
	private async _handleRetryableError(message: AssistantMessage): Promise<boolean> {
		...
		this._retryAttempt++;
		const maxAttempts = this._getRetryLimit(message, settings);
		...
		if (this._retryAttempt > maxAttempts) {
			...
			this._emit({
				...
				finalError: buildRetryFailureMessage({
					kind: classifyRetryableError(message.errorMessage || ""),
					attempts: this._retryAttempt - 1,
					errorMessage: message.errorMessage || "",
				}),
			});
			this._retryAttempt = 0;
		...
		const delayMs = this._getRetryDelayMs(message, settings);
