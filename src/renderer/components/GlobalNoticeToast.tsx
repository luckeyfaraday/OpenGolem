...
export function GlobalNoticeToast({ notice, onDismiss, onAction }: Props) {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50">
      <div className={`max-w-sm rounded-xl border bg-surface/95 backdrop-blur-sm shadow-xl ${tone.border}`}>
        ...
        {noticeAction && actionLabel && (
          <div className="px-4 pb-3">
            <button
              ...
              className="w-full rounded-lg border border-accent/40 bg-accent/10 px-3 py-2 text-sm font-medium text-accent hover:bg-accent/20 transition-colors"
            >
              {actionLabel}
            </button>
          </div>
