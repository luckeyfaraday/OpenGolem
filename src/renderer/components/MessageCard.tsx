...
const ContentBlockView = memo(function ContentBlockView({
  ...
  switch (block.type) {
    ...
    case 'file_attachment': {
      ...
      const resolvedPath = resolveFilePath(fileBlock.relativePath || fileBlock.filename);
      ...
      return (
        <button
          type="button"
          onClick={async () => {
            if (!canReveal || !window.electronAPI?.showItemInFolder) {
              return;
            }
            try {
              const revealed = await window.electronAPI.showItemInFolder(
              ...
              if (!revealed) {
                setGlobalNotice({
                  id: `message-card-file-attachment-reveal-failed-${Date.now()}`,
                  type: 'warning',
                  message: t('context.revealFailed'),
                });
              }
          ...
          className={`flex max-w-full min-w-0 items-center gap-2 px-3 py-2 rounded-lg bg-surface-muted border border-border overflow-hidden text-left ${
            canReveal ? 'hover:bg-surface-hover transition-colors' : ''
          }`}
          title={resolvedPath}
        >
